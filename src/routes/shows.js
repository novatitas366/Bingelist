...
router.get('/:id/cast', async (req, res, next) => {
  try { res.json(await getShowCast(req.showId)); } catch (e) { next(e); }
});

// --- GET /api/shows/:id/seasons ---
router.get('/:id/seasons', async (req, res, next) => {
  try { res.json(await getShowSeasons(req.showId)); } catch (e) { next(e); }
});

// --- GET /api/shows/:id/episodes ---
router.get('/:id/episodes', async (req, res, next) => {
  try { res.json(await getEpisodes(req.showId)); } catch (e) { next(e); }
});

// --- GET /api/shows/:id ---
// Declared last so the more specific /:id/cast etc. match first.
router.get('/:id', async (req, res, next) => {
  try { res.json(await getShow(req.showId)); } catch (e) { next(e); }
});
